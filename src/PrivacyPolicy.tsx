...
const content = {
  es: {
    ...
    intro: 'Esta politica describe como se recopilan y utilizan los datos cuando visitas ambuj.co.',
    sections: [
      {
  ...
  en: {
    ...
    intro: 'This policy describes how data is collected and used when you visit ambuj.co.',
    sections: [
      {
...
export default function PrivacyPolicy({ lang = 'es' }: { lang?: 'es' | 'en' }) {
  ...
  useEffect(() => {
    document.title = `${t.title} | ambuj.co`
    ...
    if (canonical) canonical.href = `https://ambuj.co/${lang === 'es' ? 'privacidad' : 'privacy'}`

    // Fix meta description
    let desc = document.querySelector('meta[name="description"]') as HTMLMetaElement
    if (desc) desc.content = lang === 'es'
      ? 'Politica de privacidad de ambuj.co. Como se recopilan y utilizan los datos del chatbot y la web.'
