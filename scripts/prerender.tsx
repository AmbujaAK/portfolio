...
let enPage = indexHtml
  ...
  .replace(/<link rel="canonical" href="[^"]*" \/>/, '<link rel="canonical" href="https://ambuj.co/en" />')
  .replace(/<meta property="og:url" content="[^"]*" \/>/, '<meta property="og:url" content="https://ambuj.co/en" />')
  ...
  .replace(/<meta name="twitter:url" content="[^"]*" \/>/, '<meta name="twitter:url" content="https://ambuj.co/en" />')
...
const aboutJsonLd = {
  ...
  mainEntity: {
    ...
    jobTitle: ['[YOUR_JOB_TITLE_1]', '[YOUR_JOB_TITLE_2]'],
    knowsAbout: [
    ...
    address: { '@type': 'PostalAddress', addressCountry: '[YOUR_COUNTRY_CODE]' },
  },
};
...
for (const lang of ['es', 'en'] as const) {
  ...
  const url = `https://ambuj.co/${slug}`;
  const altUrl = `https://ambuj.co/${altSlug}`;
  ...
  const hreflangLinks = `<link rel="alternate" hreflang="${lang}" href="${url}" /><link rel="alternate" hreflang="${altLang}" href="${altUrl}" /><link rel="alternate" hreflang="x-default" href="https://ambuj.co/sobre-mi" />`;
...
function buildArticlePage(
  ...
  const xDefaultHref = `https://ambuj.co/${config.xDefaultSlug || config.slugs.es}`;
  ...
  let result = indexHtml
    ...
    .replace(/<meta property="og:image" content="[^"]*" \/>/, `<meta property="og:image" content="${esc(config.ogImage || 'https://ambuj.co/og-image.webp')}" />`)
  ...
  if (seoMeta) {
    const articleMetaTags = [
      ...
      `<meta property="article:author" content="https://www.linkedin.com/in/[YOUR_HANDLE]" />`,
  ...
  if (seoMeta && i18n) {
    ...
    if (t) {
      const jsonLd = buildArticleJsonLd({
        lang,
        url: `https://ambuj.co/${slug}`,
        altUrl: `https://ambuj.co/${altSlug}`,
...
for (const [lang, slug, altSlug] of [['es', 'privacidad', 'privacy'], ['en', 'privacy', 'privacidad']] as const) {
  ...
  const altUrl = `https://ambuj.co/${altSlug}`;
  const altLang = lang === 'es' ? 'en' : 'es';
  const title = lang === 'es' ? 'Política de Privacidad | ambuj.co' : 'Privacy Policy | ambuj.co';
  const description = lang === 'es'
    ? 'Política de privacidad de ambuj.co. Cómo se recopilan y utilizan los datos del chatbot y la web.'
    : 'Privacy policy for ambuj.co. How chatbot and website data is collected and used.';
  ...
  const hreflangLinks = `<link rel="alternate" hreflang="${lang}" href="${url}" /><link rel="alternate" hreflang="${altLang}" href="${altUrl}" /><link rel="alternate" hreflang="x-default" href="https://ambuj.co/privacidad" />`;
...
const notFoundHtml = indexHtml
  ...
  .replace(/<meta name="robots" content="[^"]*" \/>/, '<meta name="robots" content="noindex, nofollow" />')
  .replace(/<title>[^<]*<\/title>/, '<title>404 — Page not found | ambuj.co</title>');
